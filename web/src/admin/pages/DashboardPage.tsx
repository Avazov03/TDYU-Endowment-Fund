...
import { api } from '../api'
import { BarChart, QueueMix, TrendBadge } from '../charts'
    ...
    <>
      ...
      <div className="dash-grid dash-grid-chart">
          ...
          <div className="panel-pad">
            {labels.length ? (
              <BarChart
                labels={labels}
                series={[
                  { name: 'Murojaat', values: stats?.month?.contacts || [], tone: 'cyan' },
                  { name: 'Xayriya', values: stats?.month?.donations || [], tone: 'ok' },
                  { name: 'Grant', values: stats?.month?.grants || [], tone: 'warn' },
                  { name: 'Buyurtma', values: stats?.month?.orders || [], tone: 'sky' },
                ]}
              />
            ) : (
        ...
        <div className="panel">
          ...
          <div className="panel-pad">
            <div className="queue-hero">{stats?.openWork ?? 0}</div>
            <p className="queue-caption">Tasdiq yoki javob kutayotgan yozuvlar</p>
            <QueueMix
              items={[
