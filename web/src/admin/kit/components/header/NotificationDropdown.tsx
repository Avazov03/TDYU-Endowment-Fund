...
export default function NotificationDropdown() {
  ...
  const { t } = useI18n();
  const { notices, unreadCount, markAllRead, markRead } = useDemoWorkspace();
    ...
    <div className="relative">
      ...
      >
        ...
        {notices.length === 0 ? (
          ...
          <ul className="flex flex-col gap-1 overflow-y-auto max-h-[280px]">
            {notices.map((item) => (
              <li key={item.id}>
                <Link
                  to={item.href}
                  ...
                  className={`block rounded-lg px-3 py-2.5 hover:bg-gray-50 dark:hover:bg-white/[0.04] ${
                    item.read ? "" : "bg-brand-50/60 dark:bg-brand-500/10"
                  }`}
                >
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{item.title}</p>
                  <p className="mt-0.5 text-theme-xs text-gray-500 dark:text-gray-400">{item.body}</p>
                  <p className="mt-1 text-theme-xs text-gray-400">{item.time}</p>
                </Link>
              </li>
            ))}
          </ul>
