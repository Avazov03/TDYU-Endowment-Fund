...
export default function UserDropdown() {
  ...
  const [who, setWho] = useState({ name: 'Admin', email: '' });
  const fullName = who.name;

  useEffect(() => {
    api<{ name: string; email: string }>('/api/auth/me')
      .then((u) => setWho({ name: u.name || u.email, email: u.email }))
      .catch(() => undefined)
  }, [])
    ...
    <div className="relative">
      ...
      >
        <span className="mr-3 overflow-hidden rounded-full h-11 w-11">
          <img src="/brand/endowment-seal.png" alt="User" />
        </span>

        <span className="block mr-1 font-medium text-theme-sm">{fullName || t("header.admin")}</span>
        <svg
